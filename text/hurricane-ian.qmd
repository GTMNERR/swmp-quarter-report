...
All the rainfall during the storm occurred within two days (September 28 - 29, 2022). After the storm passed into the Atlantic Ocean the night of September 29, 2022, an abrupt stop of all precipitation occurred and the system did not get any more rain until more than two weeks later on October 17, 2022. There was a total of 238mm (9.37in) of rain within those two days, the majority of which (67.5%) was on Thursday, September 29, 2022.
...
#   filter(between(datetimestamp, 
#                  as.POSIXct("2022-10-03 00:00:00"), 
#                  as.POSIXct("2022-10-10 00:00:00"))) %>%
#   mutate(depth = if_else(station == "gtmpcwq", 
#                          level + 1.11427, 
#                          depth),
#          depth = depth + 1) %>% # convert PC level to depth to be able to compare sites
#   filter(station == "gtmpcwq") %>% 
#   select(station, datetimestamp, depth, sal) %>% 
#   ggplot(aes(x = datetimestamp, y = depth)) +
#   geom_vline(xintercept = as.POSIXct("2022-09-29 14:30:00"), color = "red", size = 1) +
#   geom_line() +
#   geom_point(size = 3) +
#   geom_line(aes(y = sal)) +
#   geom_point(aes(y = sal), size = 3) +
...
gganimate::animate(sal_animation, height = 5, width = 8, units = "in", res = 120)
