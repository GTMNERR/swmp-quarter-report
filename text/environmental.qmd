...
In 2023, days ranged from 9.25 hours of daylight in the fall months (Nov-Dec) to 14.75 hours in the summer months (Jun-Aug) (@fig-daylight). Overall, the GTM weather station detected approximately 12.6 hours of daylight on average for the year. The switch on October 30, 2023 to a new type of photosynthetically active radiation (PAR) sensor on the station likely resulted in the more variable readings in the last quarter of 2023. This sensor is being looked into during annual quality checks.
...
#| label: fig-daylight
#| echo: false
#| fig.cap: "Hours of daylight based on photosynthetically active radiation data over 0 mmol per square meter collected at the Pellicer Creek weather station."
...
MET %>% 
  ...
  ggplot(aes(x = date, y = duration)) +
    geom_point(color = '#0075AC') +
  # geom_segment(aes(x = date, xend = date, y = 8, yend = duration)) +
  geom_hline(aes(yintercept = 14.75), linetype = "dashed") +
  geom_hline(aes(yintercept = 12.6)) +
  geom_hline(aes(yintercept = 9.25), linetype = "dotted") +
