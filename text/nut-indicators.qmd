...
Dissolved oxygen dropped and was lower than average from late January through mid-March at Pine Island and Pellicer Creek (@fig-do-1; @fig-do-2; @fig-do-pc). This pattern is likely correlated with the warmer than average temperatures this area experienced during this time (@fig-range-atemp; @fig-range-temp). Waters at all stations were oxygenated following rainfall events and Nor'easters (@fig-rainfall-3; @fig-do). 

Chlorophyll *a* values were higher than average at all sites in the first quarter, particularly January and March (@fig-chla-boxplots). Although too soon to make inferences on the patterns in the annual geometric means (AGM), so far the data shows elevated AGM chlorophyll *a* at all sites except Fort Matanzas (@fig-chla-agm). 

Interpretations of patterns in total nitrogen (TN) and dissolved inorganic nitrogen (DIN) are difficult this year since quite a bit of the values were not able to be calculated. Total Kjeldahl Nitrogen (TKN) was lower than average for most sites except for Pine Island where values were elevated in March (@fig-tkn-boxplots; @fig-tkn-boxplots-1).

Total phosphorus continues to fall below the state threshold for every station (@fig-tp-boxplots). Though below the state threshold, Pine Island experienced elevated TP levels in March (@fig-tp-boxplots-1). Total phosphorus and orthophosphate levels were typically much lower than average at the remaining sites during that same time period (@fig-po4-boxplots). Pine Island also had much higher orthophosphate levels for each month of the quarter than average at this site (@fig-po4-boxplots-1).
